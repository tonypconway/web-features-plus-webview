...
const { features: webFeatures } = require('web-features');
const fs = require('fs');
...
const addWebviewSupport = (feature) => {
  ...
  if (
    (compatKeysBaseline.length > 0 && androidUnsupportedBaseline.length === compatKeysBaseline.length) ||
    (compatKeysBaseline.length === 0 && (webview_support.android_unsupported_compat_features?.length ?? 0) === androidBcdKeysWithSupport)
  ) {
  ...
  if (
    (compatKeysBaseline.length > 0 && iosUnsupportedBaseline.length === compatKeysBaseline.length) ||
    (compatKeysBaseline.length === 0 && (webview_support.ios_unsupported_compat_features?.length ?? 0) === iosBcdKeysWithSupport)
  ) {
  ...
  if (
    webview_support.android === "equivalent" && webview_support.ios === "unsupported"
    ||
    webview_support.android === "unsupported" && webview_support.ios === "equivalent"
    &&
    webview_support.android === "equivalent" && webview_support.ios === "partial"
    ||
    webview_support.android === "partial" && webview_support.ios === "equivalent"
  ) {
...
fs.writeFileSync(path.resolve(__dirname, '../src/data.json'), JSON.stringify(features));
